...
| Invoicing | `com.arcanaerp.platform.invoicing` | Order-backed invoice creation, direct retrieval, paged listing, and minimal lifecycle transitions (`DRAFT -> ISSUED`, `DRAFT/ISSUED -> VOID`; `VOID` final) | `InvoiceManagement` | `POST /api/invoices`, `GET /api/invoices/{invoiceNumber}`, `GET /api/invoices`, `PATCH /api/invoices/{invoiceNumber}/status` |
