...
| Inventory | `com.arcanaerp.platform.inventory` | On-hand inventory by `sku + location`, plus location-scoped adjustment transactions | `InventoryAvailability` | `GET /api/inventory/{sku}`, `POST /api/inventory/{sku}/adjustments`, `GET /api/inventory/{sku}/adjustments` |
