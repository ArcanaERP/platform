...
| Payments | `com.arcanaerp.platform.payments` | Payment capture against issued invoices plus invoice balance and paged reconciliation reads | `PaymentManagement` | `POST /api/payments`, `GET /api/payments/invoices/{invoiceNumber}/balance`, `GET /api/payments` |
