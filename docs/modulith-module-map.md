...
| Payments | `com.arcanaerp.platform.payments` | Payment capture against issued invoices plus invoice balance, paged reconciliation reads, and tenant-scoped collection summary | `PaymentManagement` | `POST /api/payments`, `GET /api/payments/invoices/{invoiceNumber}/balance`, `GET /api/payments`, `GET /api/payments/tenants/{tenantCode}/summary` |
