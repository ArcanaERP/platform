...
| Agreements | `com.arcanaerp.platform.agreements` | Agreement master-record creation, direct retrieval by agreement number, paged listing (optional status filtering), lifecycle transitions (`DRAFT -> ACTIVE/TERMINATED`), and immutable status-change history with tenant-scoped actor/reason attribution | `AgreementManagement` | `POST /api/agreements`, `GET /api/agreements/{agreementNumber}`, `GET /api/agreements`, `PATCH /api/agreements/{agreementNumber}/status`, `GET /api/agreements/{agreementNumber}/status-history` |
