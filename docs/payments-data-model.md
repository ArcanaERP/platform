...
### WeeklyTenantCollectionsAssigneeDashboardSummary

Purpose:
- expose week-bucketed collections follow-up outcome mix by current assignee for dashboard trend reporting
...
### WeeklyTenantCollectionsClaimSummary

Purpose:
- expose week-bucketed claim intake volume by claiming actor for collections workflow analytics
...
Rules:
- route: `GET /api/payments/tenants/{tenantCode}/receivables/collections/claims/weekly-summary?page=&size=&claimedBy=&claimedAtFrom=&claimedAtTo=`
- rows are grouped by Monday-based UTC `businessWeekStart` plus `claimedBy`
...
### WeeklyTenantCollectionsReleaseSummary

Purpose:
- expose week-bucketed release volume by releasing actor for collections workflow analytics
...
Rules:
- route: `GET /api/payments/tenants/{tenantCode}/receivables/collections/releases/weekly-summary?page=&size=&releasedBy=&releasedAtFrom=&releasedAtTo=`
...
### TenantCollectionsNetIntakeActorSummary

Purpose:
- compare collections intake and release volume per actor over one tenant and one optional time window
...
Rules:
- route: `GET /api/payments/tenants/{tenantCode}/receivables/collections/net-intake/actor-summary?page=&size=&actor=&changedAtFrom=&changedAtTo=`
...
### DailyTenantCollectionsAssigneeActorEffectivenessSummary

Purpose:
- expose day-bucketed assignee-vs-actor effectiveness so leadership can see how current ownership and follow-up activity line up over time
...
Rules:
- route: `GET /api/payments/tenants/{tenantCode}/receivables/collections/assignee-actor-effectiveness/daily-summary?currencyCode=&assignedTo=&changedBy=&changedAtFrom=&changedAtTo=&page=&size=`
- rows are grouped by UTC `businessDate + assignedTo + changedBy`
...
- `GET /api/payments/tenants/{tenantCode}/receivables/collections/assignee-actor-effectiveness/monthly-summary?currencyCode=&assignedTo=&changedBy=&changedAtFrom=&changedAtTo=&page=&size=`
...
### DailyCollectionsActorFollowUpOutcomeSummary

Purpose:
- expose day-bucketed follow-up completion counts grouped by audit actor plus outcome
...
Purpose:
- expose week-bucketed follow-up completion counts grouped by audit actor plus outcome
